...
contract ERC20Token is ERC20Interface {
    ...
    constructor(
        ...
        balances[msg.sender] = _initialAmount*(10**_decimalUnits);               // The creator owns all initial tokens
        totSupply = _initialAmount*(10**_decimalUnits);                        // Update total token supply
    ...
    function transferFrom(address _from, address _to, uint256 _value)  public virtual override returns (bool success) {
        uint256 allowanceFrom = allowed[_from][msg.sender];
        require(balances[_from] >= _value && allowanceFrom >= _value,"Insufficient allowed funds for transfer source.");
        balances[_to] += _value;
        balances[_from] -= _value;
        if (allowanceFrom < MAX_UINT256) {
    ...
    function totalSupply() public view virtual override returns (uint256 totSupp) {
        return totSupply;
    }
}